    ...
    <div class="form-con">
      <el-form ref="form"
        ...
        <el-form-item label="公告标题：">
          <div class="input-itm">
            <el-input placeholder="请输入公告标题" type="text"
  ...
  methods: {
    save() {
      if (this.$route.query.id) {
        this.noticeUpdateOne()
      } else {
        this.noticeCreateOne()
      }
    },
    noticeGetOne(data) {
      if (!this.$route.query.id) {
      ...
      api.noticeGetOne({params: {_id: this.$route.query.id}, method: 'get'}).then((res) => {
        if (res.data.code === 0) {
          let {
            columnId
          } = res.data.data
    ...
    noticeCreateOne() {
      api.noticeCreateOne({data: this.params}).then((res) => {
        if (res.data.code === 0) {
          this.$router.go(-1)
        }
      }).catch(() => {
    ...
    noticeUpdateOne() {
      api.noticeUpdateOne({data: this.params, params: {_id: this.$route.query.id}}).then((res) => {
        if (res.data.code === 0) {
          this.$router.go(-1)
        }
      }).catch(() => {
