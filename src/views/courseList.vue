<template>
  <mainContainer>
    ...
    <div class="table-btn-con">
      <i class="el-icon-circle-plus-outline" @click="$router.push(routerPath.courseCreatePath)"></i>
    </div>
          ...
          <template slot-scope="scope">
            <div class="btn-con">
              <i class="el-icon-edit-outline"
                 @click="$router.push(`${routerPath.courseCreatePath}?id=${scope.row._id}`)"></i>
...
export default {
  ...
  methods: {
    ...
    courseGetList() {
      api.courseGetList({params: this.params, method: 'get'}).then((res) => {
        if (res.data.code === 0) {
